...
text <- text %>% rename(Q17 = "Q17..Do.you.have.any.other.feedback.about.the.iPad?" )
text_c <- text %>% select(Q17) %>% filter(Q17 !="-")
...
ggsave("./output/Q17_wordpairfreq.png",width = 20, height = 20, dpi=300,units ="cm")
...
ggplot(data=d_w[1:20,],mapping=aes(x=word,y=freq)) + geom_col(fill="deepskyblue") +
  ...
  theme(text=element_text(size=18))
ggsave("./output/Q17_wordsinglefreq.png",width = 20, height = 20, dpi=300,units ="cm")
